...
function createProfessionalYearDropdown() {
    ...
    // Find the year select element to replace
    ...
    // Get current actual year for highlighting
    const now = new Date();
    ...
    const dropdownHTML = `
        <div class="year-dropdown-container">
            ...
            <div class="year-dropdown-menu">
                ${years.map(year => `
                    <div class="year-option ${year.value == currentYear ? 'selected' : ''} ${year.value === currentActualYear ? 'current' : ''}" 
    ...
    // Replace the old select with new dropdown
    yearSelect.parentNode.insertAdjacentHTML('afterend', dropdownHTML);
    yearSelect.style.display = 'none'; // Hide original select
    ...
    const arrow = container.querySelector('.year-dropdown-arrow');
    ...
    function openDropdown() {
        ...
        // Add subtle entrance animation delay for options
        options.forEach((option, index) => {
            option.style.animationDelay = `${index * 0.02}s`;
            option.style.animation = 'slideInUp 0.3s ease forwards';
        });
        
        // Focus management - scroll to selected option
    ...
    // Handle year selection - NO AUTO-UPDATE
    function selectYear(event) {
        ...
        // Update hidden select (but DON'T trigger change event)
        yearSelect.value = value;
        
        // Store selection globally for when user clicks "Predict"
        window.selectedYear = parseInt(value);
    ...
    return {
        ...
        setValue: (value) => {
            ...
            if (option) {
                selectYear({ currentTarget: option });
            }
        },
        getValue: () => window.selectedYear || parseInt(yearSelect.value)
    };
}

async function updatePollutantData() {
    try {
        // Get current selections from dropdowns or fallback to current values
        const yearSelect = document.getElementById("year-select");
        const monthSelect = document.getElementById("month-select");

        currentYear = yearSelect ? parseInt(yearSelect.value) : currentYear;
        currentMonth = window.selectedMonth || (monthSelect ? parseInt(monthSelect.value) : currentMonth);
        currentPollutant = window.selectedPollutant || currentPollutant;

        // Get filter from buttons
        ...
        // Update highest concentration
        if (data.highest_concentration && Array.isArray(data.highest_concentration)) {
            console.log("🏆 Updating highest concentration with real data");
            updateHighestConcentration(data.highest_concentration);
        } else {
            console.warn("❌ Highest concentration data missing");
        }
        ...
        pollutantChart = new Chart(ctx, {
            ...
            data: {
                    ...
                    datasets: [{
                        ...
                        borderSkipped: false, // full rounded caps
                        borderWidth: 2
                }]
            },
            options: {
...
function updateHighestConcentration(highestData) {
    ...
    highestData.forEach((item) => {
        const colorClass = getConcentrationColor(item.pollutant, item.concentration);
...
// FIXED: Calendar generation with CSS-only tooltips
function updateMonthlyCalendar(calendarData, monthYear) {
    ...
    // Calculate layout properly
    const firstDay = new Date(currentYear, currentMonth - 1, 1).getDay();
    const daysInMonth = calendarData.length;
    const totalCells = firstDay + daysInMonth;
    const weeksNeeded = Math.ceil(totalCells / 7);
    
    console.log(`📊 Calendar layout: First day: ${firstDay}, Days: ${daysInMonth}, Weeks needed: ${weeksNeeded}`);
    ...
    console.log(`📅 Calendar is showing: ${currentYear}-${currentMonth}`);

    // Generate calendar days with CSS-only tooltips
    calendarData.forEach((dayData) => {
        ...
        // Better icon system with fallbacks
...
function getGifAnimation(aqi) {
    console.log(`🎨 Getting animation for AQI: ${aqi}`);
    
    if (aqi <= 50) {
        return `<div class="aqi-icon-container">
            <img src="./images/sun1-ezgif.com-gif-maker.gif" 
                 alt="Good AQI" 
                 class="aqi-gif-icon" 
                 onload="console.log('☀️ Sun GIF loaded')" 
                 onerror="this.style.display='none'; this.nextElementSibling.style.display='block';" />
            <div class="emoji-fallback" style="display: none;">☀️</div>
        </div>`;
    } else if (aqi <= 100) {
        return `<div class="aqi-icon-container">
            <img src="./images/cloudy-ezgif.com-gif-maker.gif" 
                 alt="Moderate AQI" 
                 class="aqi-gif-icon" 
                 onload="console.log('⛅ Cloudy GIF loaded')" 
                 onerror="this.style.display='none'; this.nextElementSibling.style.display='block';" />
            <div class="emoji-fallback" style="display: none;">⛅</div>
        </div>`;
    } else if (aqi <= 150) {
        return `<div class="aqi-icon-container">
            <img src="./images/clouds-ezgif.com-gif-maker.gif" 
                 alt="Unhealthy AQI" 
                 class="aqi-gif-icon" 
                 onload="console.log('🌫️ Clouds GIF loaded')" 
                 onerror="this.style.display='none'; this.nextElementSibling.style.display='block';" />
            <div class="emoji-fallback" style="display: none;">🌫️</div>
        </div>`;
    } else {
        return `<div class="aqi-icon-container">
            <img src="./images/thunder-ezgif.com-gif-maker.gif" 
                 alt="Dangerous AQI" 
                 class="aqi-gif-icon" 
                 onload="console.log('⛈️ Thunder GIF loaded')" 
                 onerror="this.style.display='none'; this.nextElementSibling.style.display='block';" />
            <div class="emoji-fallback" style="display: none;">⛈️</div>
        </div>`;
    }
...
function getCorrectAQIColors(aqi) {
    console.log(`🎨 Getting colors for AQI: ${aqi}`);
...
// FIXED: Event listeners and setup functions
function setupPollutantEventListeners() {
    console.log("🔧 Setting up event listeners...");

    // Setup filter buttons only - remove setupCustomDropdown call
    setupFilterButtons();
...
function createProfessionalMonthDropdown() {
    ...
    const currentMonthName = getMonthName(parseInt(currentMonth));
    ...
    const dropdownHTML = `
        <div class="month-dropdown-container">
            ...
            <div class="month-dropdown-menu">
                ${months.map(month => `
                    <div class="month-option ${month.value == currentMonth ? 'selected' : ''} ${month.value === currentActualMonth ? 'current' : ''}" 
    ...
    function selectMonth(event) {
        ...
        window.selectedMonth = parseInt(value);
    }
    
    trigger.addEventListener('click', toggleDropdown);
    options.forEach(option => {
        option.addEventListener('click', selectMonth);
    });
...
// PROFESSIONAL POLLUTANT DROPDOWN
function createProfessionalPollutantDropdown() {
        ...
        <div class="pollutant-dropdown-container">
            ...
            <div class="pollutant-dropdown-menu">
                ${pollutants.map(pollutant => `
                    <div class="pollutant-option ${pollutant.value === currentPollutantValue ? 'selected' : ''}" 
                         data-value="${pollutant.value}">
    ...
    function openDropdown() {
        trigger.classList.add('active');
        menu.classList.add('show');
        console.log("🧪 Pollutant dropdown opened");
    }
    
    function closeDropdown() {
        trigger.classList.remove('active');
        menu.classList.remove('show');
        console.log("🧪 Pollutant dropdown closed");
    }
    
    function selectPollutant(event) {
        const option = event.currentTarget;
        const value = option.getAttribute('data-value');
        ...
        if (typeof currentPollutant !== 'undefined') {
    ...
    trigger.addEventListener('click', toggleDropdown);
    options.forEach(option => {
        option.addEventListener('click', selectPollutant);
    });
...
function addPollutantDateControls() {
    ...
    if (mainElement && !document.getElementById("pollutant-date-controls")) {
                ...
                <div style="display: flex; align-items: center; gap: 16px; flex-wrap: wrap;">
                    ...
                    <div style="display: flex; align-items: center; gap: 8px;">
                        ...
                        <select id="month-select" style="display: none;">
                            <option value="1" ${currentDateMonth === 1 ? 'selected' : ''}>January</option>
                            <option value="2" ${currentDateMonth === 2 ? 'selected' : ''}>February</option>
                            <option value="3" ${currentDateMonth === 3 ? 'selected' : ''}>March</option>
                            <option value="4" ${currentDateMonth === 4 ? 'selected' : ''}>April</option>
                            <option value="5" ${currentDateMonth === 5 ? 'selected' : ''}>May</option>
                            <option value="6" ${currentDateMonth === 6 ? 'selected' : ''}>June</option>
                            <option value="7" ${currentDateMonth === 7 ? 'selected' : ''}>July</option>
                            <option value="8" ${currentDateMonth === 8 ? 'selected' : ''}>August</option>
                            <option value="9" ${currentDateMonth === 9 ? 'selected' : ''}>September</option>
                            <option value="10" ${currentDateMonth === 10 ? 'selected' : ''}>October</option>
                            <option value="11" ${currentDateMonth === 11 ? 'selected' : ''}>November</option>
                            <option value="12" ${currentDateMonth === 12 ? 'selected' : ''}>December</option>
                        </select>
                    </div>
                    
                    <button class="predict-button" onclick="updatePollutantDateSelection()">
                        Predict
                    </button>
                </div>
            </div>
        ...
        // Initialize ALL THREE dropdowns after DOM is ready
        setTimeout(() => {
            addDropdownAnimations();
            createProfessionalYearDropdown();   // NEW: Year dropdown
            createProfessionalMonthDropdown();  // Month dropdown
            createProfessionalPollutantDropdown(); // Pollutant dropdown
        }, 100);
...
async function updatePollutantDateSelection() {
    ...
    // Get selections from ALL dropdowns or fallback
    const selectedYear = window.selectedYear || (yearSelect ? parseInt(yearSelect.value) : currentYear);
    const selectedMonth = window.selectedMonth || (monthSelect ? parseInt(monthSelect.value) : currentMonth);
    ...
    console.log(`Year: ${currentYear}, Month: ${getMonthName(currentMonth)}`);
    console.log(`Pollutant: ${currentPollutant}`);
    ...
    try {
        ...
        // Update data with all selections
        await updatePollutantData();
        hidePollutantLoading();
...
function addDropdownAnimations() {
    ...
    style.textContent = `
        ...
        .month-option, .pollutant-option {
            opacity: 0;
            animation-fill-mode: forwards;
        }
        
        .month-dropdown-menu.show .month-option,
        .pollutant-dropdown-menu.show .pollutant-option {
            opacity: 1;
        }
    `;
